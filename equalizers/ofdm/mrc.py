...
class MaximumRationCombiningEqualizer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        # input shape = [batch_size, num_ofdm_symb, fft_size] ; [batch_size, num_ofdm_symb, num_effective_subcarriers] ; broadcastable to [batch_size, num_data_symb] as tf.float32
        x, channel, n0 = inputs

        # normalize the channels
        channel = channel / tf.complex(tf.reduce_sum(tf.abs(channel)**2, axis=-1, keepdims=True), 0.)
        ...
        n0_eff = n0 # TODO: n0 is not updated yet. Implement when needed.
