...
class BiasedLMMSEOneTapEqualizer(tf.keras.layers.Layer):
    ...
    def __init__(self, resource_grid, **kwargs):
        ...
        # TODO: Provide the actual signal power, make it a Variable
        self._signal_pwr_lin = 1.
    ...
    def call(self, inputs):
        ...
        # out shape = [batch_size, num_data_symb]
        x = x * ( tf.math.conj(channel) / tf.cast((tf.square(tf.abs(channel)) + (n0 / self._signal_pwr_lin)), dtype=tf.complex64) )
        n0_eff = (n0 * tf.cast(tf.square(tf.abs(channel)), tf.float32)) / (tf.square(n0 + tf.cast(tf.square(tf.abs(channel)), tf.float32)))
...
class UnbiasedLMMSEOneTapEqualizer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        # out shape = [batch_size, num_data_symb]
        x = x * tf.math.divide_no_nan(tf.math.conj(channel), tf.cast(tf.square(tf.abs(channel)), dtype=tf.complex64))
        n0_eff = tf.math.divide_no_nan(n0, tf.cast(tf.square(tf.abs(channel)), tf.float32))
