...
from metrics import BitErrorRate, BlockErrorRate, BinomialProportionConfidenceInterval, BlockErrorCount, SymbolErrorRate, BinomialProportionConfidenceInterval_SER, MeanSampleOffset, HistogramSampleOffset, ShiftBlockErrorRate, MeanFractionalFrequencyOffset, HistogramFractionalFrequencyOffset
...
class OFDM_CCSK_TC_Model(tf.keras.Model):
    ...
    def __init__(self, name, resource_grid, root_sequence, num_bits_per_ccsk_sequence, num_ccsk_sequence_per_frame, ccsk_scheme, add_awgn, channel_model, 
                 channel_estimator, channel_interpolator, channel_equalizator, synchronizer, max_frequency_offset=0., ccsk_demodulator='Standard', l_min=0, l_max=1, u=1, oversampling=4, **kwargs):
        ...
        self._synchronizer_mode = synchronizer
        self._max_frequency_offset = max_frequency_offset
        ...
        self._index_list = []
        for current_index in tf.range(0, (2*self._resource_grid.num_ofdm_symbols)*(self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length)*self._oversampling):
            self._index_list.append(tf.range(current_index, current_index + self._resource_grid.num_ofdm_symbols*(self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length)*self._oversampling, delta=(self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length)*self._oversampling, dtype=tf.int32))

        self._true_frame_start = self._resource_grid.num_ofdm_symbols * (self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length) * self._oversampling - self._l_min
        
        # For CCSK schemes B. In case of channel estimation, they only work with block type pilots.
        if (ccsk_scheme == 'B'):
    ...
    def call(self, inputs):
        ...
        true_frequency_offset = tf.random.uniform(shape=[tf.shape(inputs)[0], 1], minval=-self._max_frequency_offset, maxval=self._max_frequency_offset, dtype=tf.float32) # out shape = [batch_size, 1]
        # true_frequency_offset = tf.zeros(shape=[tf.shape(inputs)[0], 1], dtype=tf.float32) # No Frequency Offset
        x = x * tf.math.exp(tf.complex(0., 2 * np.pi * true_frequency_offset * tf.range(0, self._resource_grid.num_ofdm_symbols * (self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length) * self._oversampling + self._l_tot - 1, dtype=tf.float32) / (self._resource_grid.fft_size * self._oversampling))) # out shape = [batch_size, num_ofdm_symbols * (fft_size + cyclic_prefix_length) * oversampling]

        # Add zero-padding for time synchronization
        # out shape = [batch_size, 3 * (num_ofdm_symbols * (fft_size + cyclic_prefix_length) * oversampling)]
        x = tf.pad(x, paddings=[[0,0],[self._resource_grid.num_ofdm_symbols * (self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length) * self._oversampling, self._resource_grid.num_ofdm_symbols * (self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length) * self._oversampling - self._l_tot + 1]])

        # Add AWGN
        if self._add_awgn:
            x = self.awgn_applier([x, self._w_n0]) # out shape = [batch_size, 3 * (num_ofdm_symbols * (fft_size + cyclic_prefix_length) * oversampling)]
        ...
        if self._synchronizer_mode == 'perfect':
            frame_start = tf.repeat(tf.expand_dims(tf.expand_dims(self._true_frame_start, axis=0), axis=0), repeats=tf.shape(inputs)[0], axis=0) # out shape = [batch_size, 1]
            fractional_frequency_offset = true_frequency_offset - tf.math.round(true_frequency_offset) # out shape = [batch_size, 1]
        elif self._synchronizer_mode == 'cp_corr':
            ## NOTATION: total_samples_frame = 3 * (num_ofdm_symbols * (fft_size + cyclic_prefix_length) * oversampling)
            # Individual correlation
            res = x[:,:-(self._resource_grid.fft_size * self._oversampling)] * tf.math.conj(x[:,self._resource_grid.fft_size * self._oversampling:]) # out shape = [batch_size, total_samples_frame - (fft_size * oversampling)]
            # Artificial tensor to enable a windowed cumsum
            tmp = tf.concat([tf.zeros(shape=[tf.shape(inputs)[0], self._resource_grid.cyclic_prefix_length * self._oversampling], dtype=tf.complex64), res[:,:-(self._resource_grid.cyclic_prefix_length * self._oversampling)]], axis=-1)  # out shape = [batch_size, total_samples_frame - (fft_size * oversampling)]
            # Windowed cumsum, over (cyclic_prefix_length * oversampling) samples. Removing the first ((cyclic_prefix_length * oversampling) - 1) samples to get to the first complete window.
            res_tmp = tf.math.cumsum(res - tmp, axis=-1)[:,(self._resource_grid.cyclic_prefix_length * self._oversampling - 1):] # out shape = [batch_size, total_samples_frame - ((fft_size + cyclic_prefix_length) * oversampling) + 1]
            res_tmp = tf.gather(res_tmp, indices=self._index_list, axis=-1) # out shape = [batch_size, total_samples_frame - ((fft_size + cyclic_prefix_length) * oversampling), num_ofdm_symbols]
            # Compute frame start as argmax of the abs
            frame_start = tf.expand_dims(tf.math.argmax(tf.reduce_mean(tf.abs(res_tmp), axis=2), axis=-1, output_type=tf.int32), axis=1) # out shape = [batch_size, 1]
            # frame_start = tf.repeat(tf.expand_dims(tf.expand_dims(self._true_frame_start, axis=0), axis=0), repeats=tf.shape(inputs)[0], axis=0) # out shape = [batch_size, 1]
            # Compute fractional frequency offset as the angle of the best position
            fractional_frequency_offset = -1 * tf.math.angle(tf.reduce_mean(tf.gather(res_tmp, indices=frame_start, axis=1, batch_dims=1), axis=2)) / (2 * np.pi) # out shape = [batch_size, 1]
            # fractional_frequency_offset = tf.zeros(shape=[tf.shape(inputs)[0], 1], dtype=tf.float32) # out shape = [batch_size, 1]

        ## Gather the synched frame for each batch
        frame_range = frame_start + tf.expand_dims(tf.range(start=0, limit=self._resource_grid.num_ofdm_symbols * (self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length) * self._oversampling, dtype=tf.int32), axis=0)
        x = tf.gather(x, indices=frame_range, axis=-1, batch_dims=1) # out shape = [batch_size, num_ofdm_symbols * (fft_size + cyclic_prefix_length) * oversampling]

        ## Apply frequency offset correction 
        x = x * tf.math.exp(tf.complex(0., -2 * np.pi * fractional_frequency_offset * tf.range(0, self._resource_grid.num_ofdm_symbols * (self._resource_grid.fft_size + self._resource_grid.cyclic_prefix_length) * self._oversampling, dtype=tf.float32) / (self._resource_grid.fft_size * self._oversampling))) # out shape = [batch_size, num_ofdm_symbols * (fft_size + cyclic_prefix_length) * oversampling]
        ...
        # Compute fractional frequency offset estimation error in [-0.5, 0.5[
        fractional_frequency_offset_error = fractional_frequency_offset - (true_frequency_offset - tf.math.round(true_frequency_offset))
        fractional_frequency_offset_error = fractional_frequency_offset_error - tf.math.round(fractional_frequency_offset_error)

        outputs = [x, frame_start, fractional_frequency_offset_error] # out shape = [batch_size, num_ccsk_per_frame * num_bits_per_ccsk_sequence], [batch_size, 1]

        return outputs

    def test_step(self, data):
        input_bits = data
        received_bits = self(data)
        self.compiled_metrics.update_state([input_bits, tf.constant(self._true_frame_start), tf.constant(0.)], received_bits)
        # TF automatically add the prefixe "output_X_" to the metrics name in case of multiple output. We remove it by slicing the metric name.
...
def ser_ci_condition(_, logs):
    if 'ser_confidence_interval' in logs:
        ...
        return (ci_span)/(ser+epsilon) < 0.05
    else:
        return False
...
def evaluate_model(model, eval_eb_n0_db, eval_length, eval_batch_size, num_bits_per_ccsk_sequence, ccsk_sequence_length, coderate, resource_grid, oversampling):
    ...
    df_histogram_sample_offset = pd.DataFrame(index=eval_eb_n0_db.numpy())
    df_histogram_sample_offset.index.name = 'Eb/N0'

    df_histogram_fractional_frequency_offset = pd.DataFrame(index=eval_eb_n0_db.numpy())
    df_histogram_fractional_frequency_offset.index.name = 'Eb/N0'
    ...
    df.insert(loc=len(df.columns), column=(model.name, 'sample_offset', 'mean'), value=np.NaN, allow_duplicates=False)
    df.insert(loc=len(df.columns), column=(model.name, 'fractional_frequency_offset', 'mean'), value=np.NaN, allow_duplicates=False)
    ...
    for (eb_n0_db, snr_db, eval_n0) in zip(eval_eb_n0_db, eval_snr_db, eval_n0):
        ...
        termination_callback = BatchTerminationCallback(sbler_ci_condition)
        ...
        bler = summary['bler']
        sbler = summary['sbler']
        mean_sample_offset = summary['mean_sample_offset']
        mean_fractional_frequency_offset = summary['mean_fractional_frequency_offset']
        ...
        df.loc[eb_n0_db.numpy(), (model.name)] = [ber, ber_ci_span, ser, ser_ci_span, bler, bler_ci_span, sbler, sbler_ci_span, mean_sample_offset, mean_fractional_frequency_offset]
        print(f'Eb/N0: {eb_n0_db}dB (SNR: {snr_db}dB) BER: {ber} SER: {ser} BLER: {bler} SBLER: {sbler}')
        print(f'Mean sample offset: {mean_sample_offset} Mean fractional frequency offset: {mean_fractional_frequency_offset}')

        [histogram_sample_offset, histogram_bins] = summary['histogram_sample_offset']
        df_histogram_sample_offset.loc[eb_n0_db.numpy(), pd.MultiIndex.from_product([[model.name], histogram_bins])] = histogram_sample_offset

        [histogram_fractional_frequency_offset, histogram_bins] = summary['histogram_fractional_frequency_offset']
        df_histogram_fractional_frequency_offset.loc[eb_n0_db.numpy(), pd.MultiIndex.from_product([[model.name], histogram_bins])] = histogram_fractional_frequency_offset
    
    # store results and model
    return (df, df_histogram_sample_offset, df_histogram_fractional_frequency_offset)
...
def main():
    ...
    ## Results Storage
    results_directory = os.path.join('./', 'study_test')
    os.makedirs(results_directory, exist_ok=True)
    

    ## OFDM Setup
    subcarrier_spacing = 15e3 # Hz
    num_ofdm_symbols = 20 # in the frame
    ...
    max_frequency_offset = 2.5 # normalized by the subcarrier spacing
    oscillator_accuracy = (max_frequency_offset * subcarrier_spacing / carrier_frequency) * 1e6 # ppm
    print(f'Oscillator Accuracy: {oscillator_accuracy} ppm')
    

    ## CCSK Modulation
    num_bits_per_ccsk_sequence = 6
    u = 1
    ...
    eval_eb_n0_db = tf.range(start=2, limit=11, delta=1, dtype=tf.float32)
    # eval_eb_n0_db = tf.constant([50.])
    eval_batch_size = 16
    ...
    ## Model Names
    TC_A_Id_PS_model_name =  f'CCSKA-NoEq-PerfectSynch'
    TC_A_Id_CPS_model_name = f'CCSKA-NoEq-CPcorr'
    
    l_min = -2
    l_max = 7

    channel_interpolator='NN1D'

    TC_A_Id_PS_model = OFDM_CCSK_TC_Model(name=TC_A_Id_PS_model_name, resource_grid=resource_grid, root_sequence=zc0, num_bits_per_ccsk_sequence=num_bits_per_ccsk_sequence,
                                          num_ccsk_sequence_per_frame=num_ccsk_per_frame, ccsk_scheme='A', add_awgn=True, channel_model=None, channel_estimator=None, 
                                          channel_interpolator=None, channel_equalizator='Id', synchronizer='perfect', max_frequency_offset=max_frequency_offset, 
                                          l_min=l_min, l_max=l_max, oversampling=oversampling)

    TC_A_Id_CPS_model = OFDM_CCSK_TC_Model(name=TC_A_Id_CPS_model_name, resource_grid=resource_grid, root_sequence=zc0, num_bits_per_ccsk_sequence=num_bits_per_ccsk_sequence,
                                           num_ccsk_sequence_per_frame=num_ccsk_per_frame, ccsk_scheme='A', add_awgn=True, channel_model=None, channel_estimator=None, 
                                           channel_interpolator=None, channel_equalizator='Id', synchronizer='cp_corr', max_frequency_offset=max_frequency_offset, 
                                           l_min=l_min, l_max=l_max, oversampling=oversampling)


    model_list = [TC_A_Id_PS_model, TC_A_Id_CPS_model]
    ...
    histogram_min_val = -100
    histogram_max_val = +100

    time_synch_metrics=[
        MeanSampleOffset(name='mean_sample_offset'), 
        HistogramSampleOffset(name='histogram_sample_offset', max_neg_offset=histogram_min_val, max_pos_offset=histogram_max_val)
    ]

    nb_bins = 101

    freq_synch_metrics=[
        MeanFractionalFrequencyOffset(name='mean_fractional_frequency_offset'), 
        HistogramFractionalFrequencyOffset(name='histogram_fractional_frequency_offset', nb_bins=nb_bins)
    ]

    for model in model_list:
        model.compile(metrics=[data_metrics, time_synch_metrics, freq_synch_metrics], run_eagerly=EAGER)
        ...
        evaluation_results[0].to_hdf(results_file, key='results')

        histogram_sample_offset_file = os.path.join(results_directory, f'histogram_sample_offset_{model._name}.h5')
        if os.path.isfile(histogram_sample_offset_file):
            print(f'\nFile already exists! Removing previous results file {histogram_sample_offset_file}\n')
            os.remove(histogram_sample_offset_file)

        evaluation_results[1].to_hdf(histogram_sample_offset_file, key='results')

        histogram_fractional_frequency_offset_file = os.path.join(results_directory, f'histogram_fractional_frequency_offset_{model._name}.h5')
        if os.path.isfile(histogram_fractional_frequency_offset_file):
            print(f'\nFile already exists! Removing previous results file {histogram_fractional_frequency_offset_file}\n')
            os.remove(histogram_fractional_frequency_offset_file)

        evaluation_results[2].to_hdf(histogram_fractional_frequency_offset_file, key='results')

        reset_metrics(data_metrics)
        reset_metrics(time_synch_metrics)
        reset_metrics(freq_synch_metrics)


        # Analytical Evaluation
        analytical_model_name = 'ccsk_awgn_analytic'
